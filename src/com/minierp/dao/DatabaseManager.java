...
// Méthode main de test pour l'Étape 1
public static void main(String[] args) {
    ...
    try {
        ...
        if (count == 0) {
            System.out.println("Aucun client trouvé dans la table 'customers'. Vérifiez que la table existe et contient des données, ou que le script ERP.sql a bien été exécuté.");
        }
        ...
        try { if (rs != null) rs.close(); } catch (SQLException e) { /* ignoré */ }
        try { if (stmt != null) stmt.close(); } catch (SQLException e) { /* ignoré */ }
        try { if (conn != null) conn.close(); } catch (SQLException e) { /* ignoré */ }
